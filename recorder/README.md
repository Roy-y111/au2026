...
**[⬇ au2026rec-0.1.4-win64.zip](https://github.com/shuotao/au2026/releases/latest)** — 免安裝，解壓縮後點兩下 `au2026rec.exe`，選 **1** 走引導設定。不需要 Python。
...
> ### 使用者自負全部風險 —— 包含 Autodesk 帳號被停權
>
> 本工具會用你的登入狀態自動開課、錄影、下載影音與附件，**這些行為有可能違反
> Autodesk University 的使用條款**，後果包括帳號被警告、限制、停權或永久終止。
> **你自己決定要不要用，你自己承擔全部後果。**
> 任何違法、侵權或違反服務條款的使用，與開發者、貢獻者、散布者**完全無關**。

重點摘要：

* 課程影音、簡報、講義、字幕的著作權屬 **Autodesk 及各場次講者所有**，本工具不取得、
  不主張任何權利。取得的檔案（含 `srt` 產出的逐字稿與翻譯）**僅供你本人學習複習**，
  不得散布、上傳、轉載、販售或公開播映。
* 你須自行確認擁有該場次的**合法觀看資格**，並自行遵守 AU 使用條款與所在地法令。
* **`download` 指令（直接下載）的條款風險明顯高於螢幕錄影**，請自行斟酌。
* 本工具依「現狀」提供，不附任何擔保；任何損失（錄影失敗、檔案損毀、**帳號受限**）
  開發者一律不負責。

**完整條文見 [DISCLAIMER.md](DISCLAIMER.md) —— 使用前請完整讀完，使用即視為同意。**
不同意任何一點，請不要使用本工具。
...
| `srt <影片或資料夾>` | 錄好的影片 → 英文逐字稿（Groq）→ 繁中字幕（`agy`，`--model` 自選模型） |

## 十二、錄完做繁中字幕（`srt`）

錄影完成後，每位使用者用**自己的 Groq 金鑰**轉英文逐字稿，再用**自己的 agent**
（預設 Antigravity CLI `agy -p`，模型用 `--model` 指定，預設 `gemini-3.8-flash-medium`）
翻成繁體中文，輸出與影片同名的 `影片.en.srt`、`影片.zh-TW.srt`。操作步驟見
[使用說明.md](使用說明.md) 第九節。

設計沿用 study「好學生筆記」的結構保留原則：

* 翻譯的 agent **只拿到編號句子、看不到時間軸**；回傳句數必須與輸入相同，不符就重試、再拆小批，絕不接受漏句。
* 組回字幕一律用英文 SRT 的原時間軸，寫檔後讀回逐條比對，不一致就拒絕輸出。
* 先濾掉 Whisper 在靜音／掌聲時的幻聽句（*Thanks for watching*、提示詞回吐等）。
* Groq 切片結果與每個翻譯批次都落地在 `影片.srt-work\`，中斷後重跑同一指令即從斷點接續。
* 只用標準函式庫；另需 ffmpeg / ffprobe。Python 的 SSL 憑證不齊時自動改用系統 `curl` 上傳。

2026-09-14 以合成英文語音短片實測：Groq 轉出 8 句 → `gemini-3.8-flash-medium` 翻譯 8 句，
時間軸逐條一致，產品名保留英文。
...
63 個測試：時間/日期/時長解析、欄名對應、AU 官方 CSV 與 ICS、重疊排隊與直播撞期、
檔名清理、設定驗證與寫回、導頁模式與退回機制，以及字幕流程（時間碼、幻聽過濾、
批次切分、句數不符時拆批重試、時間軸一致性驗證）。
...
recorder/
  au2026rec/
    ...
    subtitle.py   錄好的影片 → Groq 英文逐字稿 → agent 翻譯 → 繁中 SRT
